...
/*
    ...
    Comment with "#" means that this code was written by the author: 
    Vladyslav Kovalets (xkoval21)
*/

#include "MK60D10.h"

#define WDOG_PERIODICAL 0x5 // # Periodical mode
#define WDOG_WINDOWED   0xD // # Windowed mode
...
/* 
    ...
    To switch the mode, comment out the line with the mode you don't want to use. (126 or 127)
*/
void WDOGInit()
{
    WDOG_STCTRLH |= WDOG_STCTRLH_WDOGEN_MASK; // # Enable watchdog
    NVIC_EnableIRQ(WDOG_EWM_IRQn); // # Enable watchdog interrupt
    WDOG_UNLOCK = 0xC520; // # Unlock watchdog register 
    WDOG_UNLOCK = 0xD928; // # Unlock watchdog register
    
    WDOG_STCTRLH = WDOG_PERIODICAL;  // # Set periodical mode 
    // WDOG_STCTRLH = WDOG_WINDOWED;    // # Set windowed mode

    WDOG_TOVALH = 0x000;    // # Set time-out value register high
    WDOG_TOVALL = 0x3e8;    // # Set time-out value register low (5 sec)

    if (WDOG_STCTRLH == WDOG_WINDOWED) // # Check if windowed mode is set
    {
        WDOG_WINH = 0x000;  // # Set window register high 
        WDOG_WINL = 0x258;  // # Set window register low (3 sec)
    }
}
...
int main(void)
{
    ...
    WDOGInit(); // # Initialize the watchdog

    while (1) {
        
        // # Refresh the watchdog if SW6 is pressed
        if (!pressed_up && !(GPIOE_PDIR & BTN_SW6))
        {   
            pressed_up = 1;
            WDOG_REFRESH = 0xA602; // # Refresh the watchdog
            WDOG_REFRESH = 0xB480; // # Refresh the watchdog
        }
